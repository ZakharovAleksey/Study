...
namespace WhiteBeltWeek2 {
  ...
  void ToDoList::goToNextMonth() {
    ...
    if (*nextMonthIter > *d_curMonthIter) {
      ...
      std::for_each(begin(d_body) + *nextMonthIter, end(d_body), [&allMonthToDoItems](std::vector<std::string>& v)
      {
        allMonthToDoItems.insert(allMonthToDoItems.end(), make_move_iterator(v.begin()), make_move_iterator(v.end()));
        v.clear();
      });
      // Update ToDo list info & add all ToDo intems to the last day of this month
      d_curMonthIter = nextMonthIter;
      d_body.resize(*d_curMonthIter);
      d_body.back().insert(
        d_body.back().end(),
  ...
  void ToDoList::Dump(const size_t i_dayId) const {
    ...
    copy(begin(d_body[i_dayId - 1]), end(d_body[i_dayId - 1]), std::ostream_iterator<std::string>(std::cout, " "));
    cout << endl;
  }
  ...
  void AvgTemperature() {
    ...
    copy(begin(hot_day_ids), end(hot_day_ids), ostream_iterator<int>(cout, " "));
  }

  void GetQueue() {
    int q{ 0 }; cin >> q;
    vector<bool> queue;
    string command{ "" }; int input_value{ 0 };
  ...
  void CountryDictionary::changeCapital(const std::string & country, const std::string & new_capital) {
  ...
  void CountryDictionary::renameCountry(const std::string & old_country, const std::string & new_country) {
    auto iterOld = d_CountryCapital.find(old_country);
    auto iterNew = d_CountryCapital.find(new_country);

    if (
      (old_country == new_country) ||
      iterOld == end(d_CountryCapital) ||
    ...
    if (iterOld != end(d_CountryCapital)) {
      ...
      cout << "Country " << old_country << " with capital " << iterOld->second << " has been renamed to " << new_country << endl;
  ...
  void CountryDictionary::dumpCountry() const {
    if (d_CountryCapital.empty()) {
      ...
      std::for_each(begin(d_CountryCapital), end(d_CountryCapital), [&](const std::pair<string, string>& p) {
  ...
  void BusStops::newBus(const std::string& i_bus, const MyVector& i_busStops) {
    ...
    std::for_each(begin(i_busStops), end(i_busStops), [&](const std::string & stop) {
  ...
  void BusStops::busesForStop(const std::string & i_stop) const {
    ...
    copy(begin(iter->second), end(iter->second), ostream_iterator<std::string>(std::cout, " "));
    cout << endl;
  }
  ...
  void BusStops::stopsForBus(const std::string & i_bus) const {
    ...
    for (const auto& stop : iter->second) {
      ...
      if (buses->second.size() == 1) {
        ...
        copy_if(begin(buses->second), end(buses->second), std::ostream_iterator<std::string>(std::cout, " "),
  ...
  void BusStops::allBuses() const {
    ...
    for (const auto& p : d_route) {
      cout << "Bus " << p.first << ": ";
      std::copy(begin(p.second), end(p.second), ostream_iterator<std::string>(std::cout, " "));
      std::cout << std::endl;
    }
  }
