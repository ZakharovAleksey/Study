...
namespace WhiteBeltW5
{
  ...
  {
    return make_tuple(left.d_year, left.d_month, left.d_day) <
           make_tuple(right.d_year, right.d_month, right.d_day);
  }
  ...
  MDate::MDate(int year, int month, int day)
      : d_year(year), d_month(month), d_day(day)
  {
  }

  void MDate::readSingleValue(stringstream& ss, int& value,
  ...
  void MDataBase::Find(const MDate& i_date) const
  {
    ...
    if (dateIter != d_body.end())
    {
      copy(dateIter->second.begin(), dateIter->second.end(),
