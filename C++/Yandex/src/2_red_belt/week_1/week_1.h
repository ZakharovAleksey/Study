...
template<class T> class Table {
  ...
  vector<T>& operator[](size_t id) { return body_[id]; }

  const vector<T>& operator[](size_t id) const { return body_[id]; }
...
template<class T> class Deque {
  ...
  T& Front() { return (!front_.empty()) ? *prev(end(front_)) : *begin(back_); }
  ...
  T& Back() { return (!back_.empty()) ? *prev(end(back_)) : *begin(front_); }
...
template<class It> class ItRange {
  private:
  It first, last;
  ...
  bool operator!=(const ItRange& other) { return !(*this == other); }
};

template<class It> ItRange<It> MakeRange(It first, It second)
{
  return { first, second };
}

template<class Container> auto Head(Container& body, size_t size)
{
...
template<class It> class Paginator {
  ...
  {
    size_t pages_count =
        static_cast<size_t>(ceil((double) distance(beg, end) / page_size));
    pages_.reserve(pages_count);

    for (size_t page_id = 0; page_id < pages_count; ++page_id)
    {
      It start = beg + page_id * page_size;
      size_t cur_page_size = (distance<It>(start, end) < page_size)
                                 ? distance<It>(start, end)
                                 : page_size;
...
template<class It> Paginator<It> MakePaginator(It beg, It end, size_t page_size)
{
