...
namespace geo2d
{
  ...
  {
    return static_cast<int64_t>(lhs.x) * rhs.y -
           static_cast<int64_t>(rhs.x) * lhs.y;
  }

  int64_t ScalarProduct(Vector lhs, Vector rhs)
  {
    return static_cast<int64_t>(lhs.x) * rhs.x +
  ...
  {
    return r.Left() <= p.x && p.x <= r.Right() && r.Bottom() <= p.y &&
           p.y <= r.Top();
  }
  ...
  bool Collide(Segment s1, Segment s2)
  {
    ...
    return Sign(v1 * Vector{ s1.p1, s2.p1 }) *
                   Sign(v1 * Vector{ s1.p1, s2.p2 }) <=
               0 &&
           Sign(v2 * Vector{ s2.p1, s1.p1 }) *
                   Sign(v2 * Vector{ s2.p1, s1.p2 }) <=
               0;
  }
  ...
  bool Collide(Circle c, Segment s)
  {
        ...
        ScalarProduct(Vector{ s.p2, s.p1 }, Vector{ s.p2, c.center }) >= 0)
    {
      ...
      // векторного произведения ниже, обозначим её 2S. Высота этого
      // треугольника, проведённая из c.center, равна 2S / |s.p1, s.p2|. Чтобы
      // остаться в целых числах, возведём сравниваемые величины в квадрат и
      // сравним (2S)^ 2 с R^2 * |s.p1, s.p2|^2
      uint64_t double_triangle_square =
          abs(Vector{ s.p1, s.p2 } * Vector{ s.p1, c.center });
      return Sqr(double_triangle_square) <=
             Sqr<uint64_t>(c.radius) * DistanceSquared(s.p1, s.p2);
    } else
    {
      auto d = std::min(DistanceSquared(c.center, s.p1),
                        DistanceSquared(c.center, s.p2));
  ...
  {
    return DistanceSquared(c1.center, c2.center) <=
