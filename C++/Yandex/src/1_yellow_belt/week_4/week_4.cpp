...
{
  auto pos = find_if(begin(numbers), end(numbers), [](const int& val) {
    return val < 0;
  });
...
vector<string> SplitIntoWords(const string& s)
{
  ...
  {
    word_end = find_if(word_begin, end(s), [](const char& symbol) {
      return symbol == ' ';
    });
...
void PrintStats(vector<Person> persons)
{
  ...
  cout << "Median age for females = "
       << ComputeMedianAge(begin(persons), f_m_pos) << endl;
  cout << "Median age for males = " << ComputeMedianAge(f_m_pos, end(persons))
       << endl;
...
bool CompareOperations(const string& prev, const string cur)
{
  ...
  return (find(begin(high), end(high), cur) != end(high) &&
          find(begin(low), end(low), prev) != end(low))
             ? true
             : false;
}
