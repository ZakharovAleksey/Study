...
{
  auto pos = find_if(begin(elements), end(elements), [border](const T& el) {
    return el > border;
  });
...
void MergeSortDiv2(RandomIt range_begin, RandomIt range_end)
{
  ...
  merge(begin(tmp), begin(tmp) + dist / 2, begin(tmp) + dist / 2, end(tmp),
        range_begin);
...
void MergeSortDiv3(RandomIt range_begin, RandomIt range_end)
{
  ...
  merge(begin(tmp), begin(tmp) + dist / 3, begin(tmp) + dist / 3,
        begin(tmp) + 2 * dist / 3, back_inserter(lol));
...
namespace t
{
  class Person {
    ...
    // получить имя и фамилию по состоянию на конец года year с помощью
    // двоичного поиска
