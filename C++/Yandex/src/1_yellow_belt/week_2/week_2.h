...
struct BusesForStopResponse
{
  ...
  BusesForStopResponse(const vector<string>& stops_to_buses)
      : stops_to_buses(stops_to_buses)
  {
  }
};
...
{
  ostringstream os;
  os << num << " / " << denom << " -> " << exp_num << " / " << exp_denom
     << endl;

  AssertEqual(Rational(num, denom).Numerator(), exp_num,
              os.str() + " numerator\n");
  AssertEqual(Rational(num, denom).Denominator(), exp_denom,
              os.str() + " denominator\n");
}
